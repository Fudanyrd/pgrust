...
//!
//! * `small1_seams::parser_errposition` — the cursor-position
//!   helper (consumed by define / cluster / explain-state).
//! * `scansup_seams::truncate_identifier` — reconciled to live
//!   here (the parser-driver's lexer is the consumer).
//! * `parser_analyze_seams::make_parsestate` — `ParseState` allocator.
//!
...
pub fn transformContainerSubscripts<'mcx>(
    ...
    let element_type = match routines {
        ...
        None => {
            ...
            let tname =
                adt_format_type::format_type_be_owned(container_type)?;
            return Err(ereport(ERROR)
    ...
    let sbsref = small1_seams::subscripting_transform::call(
        mcx,
        sbsref,
        indirection,
        pstate,
    ...
    if !OidIsValid(sbsref.refrestype) {
        let tname = adt_format_type::format_type_be_owned(container_type)?;
        return Err(ereport(ERROR)
            .errcode(ERRCODE_DATATYPE_MISMATCH)
            .errmsg(alloc::format!(
                "cannot subscript type {} because it does not support subscripting",
...
pub fn make_const<'mcx>(
    ...
    let (val, typeid, typelen, typebyval): (Datum<'mcx>, Oid, i32, bool) = match val_node.node_tag() {
        ...
        ntag::T_Float => {
            ...
            match pg_strtoint64_safe(f.fval.as_str()) {
                ...
                None => {
                    ...
                    let bytes = small1_seams::numeric_in::call(
                        mcx,
                        f.fval.as_str(),
                    )
        ...
        ntag::T_BitString => {
            ...
            // by-reference Datum.
            let bytes = small1_seams::bit_in::call(
                mcx,
...
fn pg_strtoint64_safe(s: &str) -> Option<i64> {
    ...
    match numutils::pg_strtoint64_safe(s, Some(&mut escontext)) {
...
pub fn variable_coerce_param_hook(
    ...
    if param.paramkind == PARAM_EXTERN && param.paramtype == UNKNOWNOID {
        ...
        if existing == UNKNOWNOID {
            ...
            drop(param_types);
            let was = adt_format_type::format_type_be_owned(existing)?;
            let now = adt_format_type::format_type_be_owned(target_type_id)?;
            return Err(ereport(ERROR)
...
pub fn init_seams() {
    small1_seams::parser_errposition::set(parser_errposition_seam);
    ...
    //   pos = pg_mbstrlen_with_len(p_sourcetext, location) + 1;
    collationcmds_seams::parser_errposition::set(|source, location| {
        if location < 0 {
            return 0;
        }
        match source {
    ...
    //   if (p_sourcetext == NULL) return 0;
    //   pos = pg_mbstrlen_with_len(p_sourcetext, location) + 1;
    functioncmds_seams::parser_errposition::set(|source, location| {
        if location < 0 {
            return 0;
        }
        match source {
    ...
    scansup_seams::truncate_identifier::set(truncate_identifier);
    scansup_seams::downcase_truncate_identifier::set(downcase_truncate_identifier);
    parser_analyze_seams::make_parsestate::set(make_parsestate);
    small1_seams::coerce_param_hook::set(coerce_param_hook_seam);
}
