...
use ::parse_target::FigureColname;
use ::adt_xml::map_sql_identifier_to_xml_name;
...
use ::nodes_core::makefuncs::{
    ...
    make_json_constructor_expr, make_json_format, make_json_is_predicate, make_json_value_expr,
    make_target_entry,
};
...
fn transform_a_const<'mcx>(
    ...
    let con = small1::make_const(m, &*pstate, &a)?;
...
fn transformAExprOp<'mcx>(
    ...
    if guc_tables::vars::Transform_null_equals.read()
        && is_eq_name
        && either_null
        && neither_casetest
    {
...
fn transformCollateClause<'mcx>(
    ...
    for n in collname.into_iter() {
        ...
        match other.into_string() {
            Some(s) => collname_pn.push(::parsenodes::Node::String(
    ...
    let coll_oid = parse_type::LookupCollation(
        scratch.mcx(),
        Some(&*pstate),
        &collname_pn,
        location,
...
fn make_row_comparison_op<'mcx>(
    ...
    for (larg, rarg) in largs.into_iter().zip(rargs.into_iter()) {
        let last_srf = last_srf_expr(pstate);
        let cmp_node = parse_oper::make_op(
            Some(pstate),
            &opname_s,
            Some(larg),
            Some(rarg),
...
/// (`::parsenodes::TypeName`/`Node`), distinct from the raw-grammar
/// `nodes` `TypeName`/`Node` this dispatcher carries. The `names`
...
fn typename_type_id_and_mod<'mcx>(
    ...
    for n in tn.names.iter() {
        if let Some(s) = n.as_string() {
            names.push(::parsenodes::Node::String(
                ::parsenodes::StringNode {
                    sval: Some(String::from(s.sval.as_str())),
                },
            ));
    ...
    let mut array_bounds: Vec<::parsenodes::Node> =
    ...
    for n in tn.arrayBounds.iter() {
        ...
        if let Some(i) = n.as_integer() {
            array_bounds.push(::parsenodes::Node::Integer(
                ::parsenodes::Integer { ival: i.ival },
            ));
        } else {
            array_bounds.push(::parsenodes::Node::Integer(
                ::parsenodes::Integer { ival: -1 },
            ));
        }
    }
    let tn_pn = ::parsenodes::TypeName {
        names,
        typeOid: tn.typeOid,
        setof: tn.setof,
    ...
    parse_type::typenameTypeIdAndMod(scratch.mcx(), Some(pstate), &tn_pn)
}
...
fn transformWholeRowRef<'mcx>(
    ...
    let (rte, p_rtindex, p_returning_type, names_is_eref, colnames_len, colnames) = {
        let owner = ::parser_relation::nsitem_level(pstate, sublevels_up);
    ...
    if names_is_eref || p_returning_type != ::nodes::primnodes::VarReturningType::VAR_RETURNING_DEFAULT
    {
        ...
        ::parser_relation::markNullableIfNeeded(pstate, &mut var)?;
        // Mark relation as requiring whole-row SELECT access.
        ::parser_relation::markVarForSelectPriv(mcx, pstate, &var)?;
...
fn parse_func_on_whole_row<'mcx>(
    ...
    let arg = node_into_expr(whole)
        ...
        .erase_lifetime();
    let last_srf = last_srf_expr(pstate);
    let res = func::ParseFuncOrColumn(
        pstate,
        &funcname,
        vec![arg],
        last_srf.as_ref(),
        ...
        match n_tag {
            ...
            ntag::T_String => {
                ...
                if !subscripts.is_empty() {
                    ...
                    let sref = small1::transformContainerSubscripts(
                        mcx,
                        pstate,
                        result.clone_in(mcx)?,
                        ctype,
                ...
                let funcname = [::mcx::PgString::from_str_in(&colname, mcx)?];
                let newresult = func::ParseFuncOrColumn(
                    pstate,
                    &funcname,
                    vec![result.clone()],
                    last_srf.as_ref(),
...
fn transformRowExpr<'mcx>(
    ...
    let newargs_vec = ::parse_target::transformExpressionList(
        mcx,
        pstate,
        r.args,
        expr_kind,
...
fn transformParamRef<'mcx>(
    ...
    let result: Option<::nodes::primnodes::Param> = match &pstate.p_ref_hook_state {
        ...
        ParseRefHookState::VarParams(parstate) => Some(
            small1::variable_paramref_hook(pstate, parstate, &hook_pref)?,
        ),
...
fn transformSubLink<'mcx>(
    ...
    let qtree_node =
        parser_analyze_seams::parse_sub_analyze::call(
            mcx,
            &subselect,
            pstate,
            None,
...
fn seam_transform_grouping_func<'mcx>(
    ...
    parse_agg_seams::transform_grouping_func::call(pstate, gf)
}
...
/// `getJsonEncodingConst(format)` (parse_expr.c:3249) — a NAMEOID `Const`
/// carrying the JSON encoding name (UTF8 default). A `name` is a fixed-length
/// pass-by-reference value: it crosses the by-ref lane as a NUL-padded
/// `NAMEDATALEN` buffer with no varlena header (the canonical `name` framing),
/// exactly as `convert_from`'s name arg reads it back.
fn get_json_encoding_const<'mcx>(
    ...
    let encoding = match format {
        ...
        _ => JsonEncoding::JS_ENC_UTF8,
    };

    let enc: &str = match encoding {
    ...
    // namestrcpy into a NAMEDATALEN-byte NUL-padded buffer.
    ...
    let value = Datum::ByRef(::mcx::slice_in(mcx, &buf)?);

    // makeConst(NAMEOID, -1, InvalidOid, NAMEDATALEN, NameGetDatum(encname),
    //           false, false)
...
/// `convert_from(expr, <encoding>)` decoding the bytea input per the JSON
...
fn make_json_bytea_to_text_conversion<'mcx>(
    ...
    // makeFuncExpr(F_CONVERT_FROM, TEXTOID, list_make2(expr, encoding),
    //              InvalidOid, InvalidOid, COERCE_EXPLICIT_CALL);
    // then fexpr->location = location.
    let fexpr = FuncExpr {
        ...
        inputcollid: InvalidOid,
        args: alloc::vec![expr, encoding],
        location,
    };

    Ok(Expr::FuncExpr(fexpr))
}
...
fn transformJsonArrayQueryConstructor<'mcx>(
    ...
    let qtree_node = parser_analyze_seams::parse_sub_analyze::call(
...
fn transformJsonSerializeExpr<'mcx>(
    ...
    if jsexpr.output.is_some() {
        returning = transform_json_output(pstate, jsexpr.output.as_deref(), true)?;
...
/// `transformJsonParseArg(pstate, jsexpr, format, exprtype)`
/// (parse_expr.c:4061) — prepare the input document of `JSON()` / `IS JSON`:
/// coerce the (string/bytea) argument to text. Returns the prepared expression
/// and reports the resulting type via `exprtype`.
fn transform_json_parse_arg<'mcx>(
    pstate: &mut ParseState<'mcx>,
    raw: Option<Node<'mcx>>,
    format: &Option<JsonFormat>,
    ...
    if exprtype == BYTEAOID {
        let raw_clone = raw_expr.clone_in(aexpr_clone_ctx(pstate))?;
        ...
        if exprtype == UNKNOWNOID || typcategory == TYPCATEGORY_STRING {
            e = coerce::coerce_to_target_type::call(
                pstate,
                e,
                exprtype,
                TEXTOID,
            ...
            .ok_or_else(|| {
                PgError::error("transformJsonParseArg: cannot coerce string input to text")
            })?;
            exprtype = TEXTOID;
        }

        if let Some(f) = format.as_ref() {
            if f.encoding != JsonEncoding::JS_ENC_DEFAULT {
                return Err(ereport(ERROR)
                    .errcode(ERRCODE_FEATURE_NOT_SUPPORTED)
                    .errposition(parser_errposition(pstate, f.location))
...
fn transformJsonParseExpr<'mcx>(
    ...
    let arg = if jsexpr.unique_keys {
        ...
        let (arg, arg_type) = transform_json_parse_arg(pstate, raw, &arg_ve.format)?;

        if arg_type != TEXTOID {
            return Err(ereport(ERROR)
                .errcode(ERRCODE_DATATYPE_MISMATCH)
                .errmsg("cannot use non-string types with WITH UNIQUE KEYS clause")
                .errposition(parser_errposition(pstate, jsexpr.location))
                .into_error());
        }
...
fn transformJsonAggConstructor<'mcx>(
    ...
    let aggfilter = match agg_ctor.agg_filter.as_ref() {
        Some(af) => {
            ...
            clause_seams::transform_where_clause::call(
                mcx,
                pstate,
                Some(clause),
                ParseExprKind::EXPR_KIND_FILTER,
    ...
    let node: Expr = if let Some(over) = agg_ctor.over.as_ref() {
        ...
        let finished =
            parse_agg_seams::transform_window_func_call::call(pstate, wfunc, windef)?;
        ...
        let aggref = Aggref {
            ...
            aggstar: false,
            aggvariadic: false,
            aggkind: ::parsenodes::AGGKIND_NORMAL,
            aggpresorted: false,
        ...
        let finished = parse_agg_seams::transform_aggregate_call::call(
...
fn transform_json_behavior<'mcx>(
    ...
    if let Some(b) = behavior {
        ...
        if btype == JsonBehaviorType::JSON_BEHAVIOR_DEFAULT {
            ...
            if ::vars::var::contain_var_clause(&enode) {
                return Err(ereport(ERROR)
                    .errcode(ERRCODE_DATATYPE_MISMATCH)
...
fn lsyscache_format_type_be(typid: Oid) -> PgResult<String> {
    adt_format_type::format_type_be_owned(typid)
}
...
fn analyze_one_exec_param_impl<'mcx>(
    ...
    let mut pstate_box = small1::make_parsestate(mcx, None)?;
...
pub fn init_seams() {
    ...
    small1_seams::subscripting_transform::set(subscripting_transform_impl);
    ...
    guc_tables::vars::Transform_null_equals.install(
