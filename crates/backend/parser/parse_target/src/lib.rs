...
use small1 as parse_node;

use parse_expr_seams as parse_expr;
...
fn rel_name<'a>(rd: &'a rel::RelationData<'_>) -> &'a str {
...
pub fn expandRecordVariable<'mcx>(
    ...
    if attnum == InvalidAttrNumber {
        ...
        let mut tuple_desc =
            tupdesc::CreateTemplateTupleDesc(mcx, vars.len() as i32)?;
...
/// Seam body for `target_seams::transform_target_entry`, the
...
/// Seam body for `target_seams::FigureColname` — the column-name
...
pub fn init_seams() {
    target_seams::transform_target_entry::set(transform_target_entry_seam);
    target_seams::FigureColname::set(figure_colname_seam);
