...
pub fn quote_identifier<'mcx>(mcx: Mcx<'mcx>, ident: &str) -> PgResult<PgString<'mcx>> {
    ...
    if safe {
        ...
        let kwnum =
            keywords::ScanKeywordLookup(ident, &keywords::ScanKeywords);
        if kwnum >= 0
            && keywords::keyword_category(kwnum as usize)
                != Some(keywords::KeywordCategory::Unreserved)
        {
            safe = false;
        }
    }
...
pub fn generate_relation_name_catalog<'mcx>(
    ...
    let relname = match lsyscache_seams::get_rel_name::call(mcx, relid)? {
        Some(s) => s,
        None => {
    ...
    let need_qual = if force_qual {
        ...
        !namespace_seams::relation_is_visible::call(mcx, relid)?
    };

    let nspname: Option<PgString<'mcx>> = if need_qual {
        let relnamespace = lsyscache_seams::get_rel_namespace::call(relid)?;
        lsyscache_seams::get_namespace_name_or_temp::call(mcx, relnamespace)?
    } else {
        None
    };
...
pub fn generate_operator_clause_catalog<'mcx>(
    ...
    let nspname =
        lsyscache_seams::get_namespace_name::call(mcx, operform.oprnamespace)?
            .ok_or_else(|| {
...
pub fn generate_collation_name<'mcx>(mcx: Mcx<'mcx>, collid: Oid) -> PgResult<PgString<'mcx>> {
    ...
    let nm = syscache_seams::collation_namespace_and_name::call(mcx, collid)?
    ...
    let nspname: Option<PgString<'mcx>> =
        if namespace_seams::collation_is_visible::call(mcx, collid)? {
            None
        } else {
            lsyscache_seams::get_namespace_name_or_temp::call(mcx, nm.namespace)?
        };
...
pub(crate) fn get_opclass_name<'mcx>(
    ...
    let (opcnamespace, opcmethod, opcname) =
        syscache_seams::opclass_namespace_method_name::call(mcx, opclass)?
    ...
    let is_default = oid_is_valid(actual_datatype)
        && lsyscache_seams::get_default_opclass::call(
            actual_datatype,
            opcmethod,
        )? == opclass;
    if !is_default {
        if namespace_seams::opclass_is_visible::call(mcx, opclass)? {
            ...
            let nspname =
                lsyscache_seams::get_namespace_name_or_temp::call(
                    mcx,
                    opcnamespace,
                )?;
            let qnsp = quote_identifier(mcx, nspname.as_deref().unwrap_or(""))?;
...
fn get_reloptions<'mcx>(
    ...
    let options =
        arrayfuncs_seams::deconstruct_text_array::call(mcx, reloptions)?;
...
pub(crate) fn flatten_reloptions<'mcx>(
    ...
    let token = syscache_seams::fetch_class_reloptions::call(mcx, relid)?;
    if token.is_null {
        return Ok(None);
    }
    let mut buf = alloc::string::String::new();
...
pub struct DeparseContext<'mcx> {
    ...
    /// family that renders SQL). Modeled as the owned [`stringinfo::StringInfo`]
    ...
    pub buf: stringinfo::StringInfo<'mcx>,
...
pub(crate) fn format_type_with_typemod_pub<'mcx>(
    ...
    match format_type_seams::format_type_extended::call(mcx, type_oid, typemod, 0)? {
        Some(s) => Ok(s),
...
fn get_rel_name<'mcx>(mcx: Mcx<'mcx>, relid: Oid) -> PgResult<Option<PgString<'mcx>>> {
    lsyscache_seams::get_rel_name::call(mcx, relid)
}
    ...
    for i in 0..nrte {
        ...
        let mut refname: Option<PgString<'mcx>> = {
            ...
            if rels_used.is_some()
                && !nodes_core_seams::bms_is_member::call(rtindex, rels_used)
            {
            ...
            if let Some(counter) = names_hash.get(&key).copied() {
                ...
                loop {
                    ...
                    let m = loop {
                        ...
                        refnamelen = mbutils_seams::pg_mbcliplen::call(
                            base.as_bytes(),
                            refnamelen as i32,
                            refnamelen as i32 - 1,
                        ) as usize;
...
pub fn set_relation_column_names<'mcx>(
    ...
    let real_colnames: PgVec<'mcx, Option<PgString<'mcx>>> = if rte.rtekind == RTE_RELATION {
        ...
        ruleutils_seams::ruleutils_relation_real_colnames::call(mcx, rte.relid)?
        ...
        ruleutils_seams::ruleutils_expand_function_rte_colnames::call(mcx, rte)?
    } else {
    ...
    if !colname_is_unique(colname, dpns, colinfo) {
        ...
        loop {
            ...
            let m = loop {
                ...
                colnamelen = mbutils_seams::pg_mbcliplen::call(
                    colname.as_bytes(),
                    colnamelen as i32,
                    colnamelen as i32 - 1,
                ) as usize;
...
fn pull_varnos_node<'mcx>(
    ...
    if let Some(items) = node.as_list() {
        ...
        {
            let r = var_seams::pull_varnos::call(mcx, expr)?;
            if let Some(more) = r {
                match acc {
                    None => *acc = Some(more),
                    Some(existing) => {
...
mod tests {
    ...
    fn generate_relation_name_catalog_visible_and_qualified() {
        ...
        if !lsyscache_seams::get_rel_name::is_installed() {
            lsyscache_seams::get_rel_name::set(|mcx, relid| {
        ...
        if !lsyscache_seams::get_rel_namespace::is_installed() {
            lsyscache_seams::get_rel_namespace::set(|relid| Ok(relid + 1));
        }
        if !namespace_seams::relation_is_visible::is_installed() {
            // relid 100 visible; relid 200 not visible.
            namespace_seams::relation_is_visible::set(|_mcx, relid| Ok(relid == 100));
        }
        if !lsyscache_seams::get_namespace_name_or_temp::is_installed() {
            lsyscache_seams::get_namespace_name_or_temp::set(|mcx, nspid| {
