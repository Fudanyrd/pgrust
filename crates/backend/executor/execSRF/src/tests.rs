...
fn install() {
    ...
    sort_storage_seams::tuplestore_putvalues::set(|state, _tdesc, values, nulls| {
        let store = state
            .payload_mut()
    ...
    lsyscache_seams::type_is_rowtype::set(|_typid| Ok(false));

    // TupleDescInitEntry looks up the column type's metadata; return int4's
    // (typlen=4, by-value, 'i' align, 'p' storage).
    syscache_seams::search_type_attr_info::set(|_oid| {
        Ok(Some(
            types_tuple::tupdesc::PgTypeInfo {
                typlen: 4,
                typbyval: true,
                typalign: b'i' as i8,
    ...
    // work_mem default (KB).
    init_small_seams::work_mem::set(|| 4096);
    ...
    fmgr_seams::function_call_invoke_datum::set(
...
fn generate_series_step_int4<'mcx>(
    ...
    if fcinfo.fn_extra.is_none() {
        ...
        funcapi::srf_support::init_MultiFuncCall(fcinfo)
        ...
        let funcctx = funcapi::srf_support::per_MultiFuncCall(fcinfo)
    ...
    let funcctx: &mut FuncCallContext<'mcx> =
        funcapi::srf_support::per_MultiFuncCall(fcinfo)
            .expect("per_MultiFuncCall");
    ...
    if in_range {
        ...
        funcapi::srf_support::end_MultiFuncCall(fcinfo)
...
fn install_materialize() {
    ...
    sort_storage_seams::tuplestore_gettupleslot::set(
    ...
    execTuples_seams::slot_getattr::set(|_estate, _slot, _attnum| {
